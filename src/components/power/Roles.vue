      ...
      <el-table :data="roleList" border stripe>
        ...
        <el-table-column type="expand">
          <template v-slot:default="slotProps">
            <el-row :class="['bdbottom', i1 === 0 ? 'bdtop' : '', 'vcenter']" :key="item1.id" v-for="(item1, i1) in slotProps.row.children">
              <el-col :span="5">
              ...
              <el-col :span="19">
                <el-row :class="[i2 === 0 ? '' : 'bdtop', 'vcenter']" :key="item2.id" v-for="(item2, i2) in item1.children">
                  ...
                  <el-col :span="18">
                    <el-tag type="warning" :key="item3.id" v-for="(item3) in item2.children" closable @close="removeRightById(slotProps.row, item3.id)">{{item3.authName}}</el-tag>
                  </el-col>
                </el-row>
              </el-col>
            </el-row>
        ...
        <el-table-column label="操作" width="300px"> 
          <template v-slot:default="slotProps">
            ...
            <el-button size="mini" type="danger" icon="el-icon-delete" @click="removeRoleById(slotProps.row.id)">删除</el-button>
...
export default {
  ...
  methods: {
    ...
    async removeRightById(role, rightID) {
      ...
      // this.getRolesList() 会刷新
      role.children = res.data  //不传id的原因
    },
    //展示分配权限的对话框
    ...
    //通过递归的方式获取角色下所有三级权限的id，并保存到defKeys数组
    ...
    async allotRights() {
      console.log(this);
      const keys = [
        ...this.$refs.treeRef.getCheckedKeys(),  //ES6的扩展运算符...用于合并数组
